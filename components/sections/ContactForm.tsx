  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setStatus({
          type: "error",
          message: result.error || "Something went wrong. Please try again.e",
        });
      }
    });
  };
      ...
      <form className="space-y-3 @md/form:space-y-4" onSubmit={handleSubmit}>
        <div>
          ...
          <input
            ...
            required
            disabled={isPending}
            className="w-full px-3 py-1.5 @md/form:px-4 @md/form:py-2 rounded-lg border bg-background foxus:outline-none focus:ring-2 focus:ring-primary text-sm @md/form:text-base"
          />
        </div>
